...
export function SmartSwitch() {
  ...
  if (complete) return <section className="switch-complete" aria-live="polite"><span className="switch-success"><Icon name="check" size={22} /></span><div><span className="eyebrow">SWITCH REQUEST SUBMITTED</span><h2>Your payment switch request is ready</h2><p>{selectedCharges.length} recurring {selectedCharges.length === 1 ? 'payment' : 'payments'} were selected. Some merchants may require separate confirmation before a payment moves to UFCU.</p></div></section>
  ...
  return <section className="smart-switch" aria-labelledby="smart-switch-title">
    ...
    {!connected ? <div className="bank-connect"><label htmlFor="connected-bank">Where are your payments today?</label><div className="connect-controls"><select id="connected-bank" className="input" value={bank} onChange={event => setBank(event.target.value)}><option value="">Choose a bank</option><option value="Chase">Chase</option><option value="Bank of America">Bank of America</option><option value="Wells Fargo">Wells Fargo</option><option value="Capital One">Capital One</option></select><Button disabled={!bank} onClick={() => setConnected(true)}>Connect securely <Icon name="arrow" size={17} /></Button></div><p className="switch-disclosure"><Icon name="shield" size={16} /> Secure provider connection. UFCU never sees or stores your bank sign-in.</p></div> : <div className="switch-review"><div className="connected-bank"><span className="bank-avatar">{bank.charAt(0)}</span><div><strong>{bank} connected</strong><span>We found {charges.length} recurring payments</span></div><button className="text-link" onClick={() => setConnected(false)}>Change</button></div><fieldset><legend>Choose payments to switch to UFCU</legend><div className="charge-list">{charges.map(charge => <label className={`charge-row ${selected.includes(charge.id) ? 'is-picked' : ''}`} key={charge.id}><input type="checkbox" checked={selected.includes(charge.id)} onChange={() => toggle(charge.id)} /><span className="merchant-mark" style={{ backgroundColor: charge.color }}>{charge.merchant.charAt(0)}</span><span className="charge-name"><strong>{charge.merchant}</strong><span>{charge.detail} · Monthly</span></span><strong className="charge-amount">{charge.amount}<span>/mo</span></strong></label>)}</div></fieldset><div className="switch-footer"><p><strong>{selected.length} selected</strong>{selected.length > 0 && <> · ${total.toFixed(2)}/month</>}</p><Button disabled={selected.length === 0} onClick={() => setComplete(true)}>Review switch request <Icon name="arrow" size={17} /></Button></div></div>}
  </section>
}
