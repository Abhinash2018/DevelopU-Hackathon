...
export function IdentityReviewPage() {
  const { profile, financialBackground, preferences, selectedCard, consentAccepted, setConsentAccepted, setOnboardingStage } = useMember()
  const navigate = useNavigate()
  ...
  return <FlowShell step="03 of 04" eyebrow="SECURE INFORMATION REVIEW" title="Review before we continue." description="Check the information you entered and choose what happens next.">
    ...
    <section className="flow-review-card" aria-labelledby="personal-review-title"><div className="flow-card-title"><div><p className="member-eyebrow">PERSONAL INFORMATION</p><h2 id="personal-review-title">Your member profile</h2></div><button type="button" className="member-text-button" onClick={() => navigate('/signup')}>Edit</button></div><div className="flow-detail-grid"><ReviewDetail label="Full name" value={`${profile.firstName} ${profile.lastName}`} /><ReviewDetail label="Date of birth" value={formatDate(profile.dateOfBirth)} /><ReviewDetail label="Email" value={profile.email} /><ReviewDetail label="Phone" value={profile.phone} /><ReviewDetail label="Address" value={`${profile.street}, ${profile.city}, ${profile.state} ${profile.zip}`} /><ReviewDetail label="Identification" value={`${profile.identificationType ? identificationLabels[profile.identificationType] : 'Not provided'} · •••• ${profile.identificationLast4}`} /><ReviewDetail label="SSN" value={`•••• ${profile.ssnLast4}`} /><ReviewDetail label="Employment or student status" value={financialBackground.employmentStatus || 'Not provided'} /><ReviewDetail label="Annual income range" value={financialBackground.annualIncomeMin !== '' && financialBackground.annualIncomeMax !== '' ? `${Number(financialBackground.annualIncomeMin).toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 2 })} – ${Number(financialBackground.annualIncomeMax).toLocaleString('en-US', { style: 'currency', currency: 'USD', maximumFractionDigits: 2 })}` : 'Not provided'} /><ReviewDetail label="Debit card design" value={({ texas: "The University of Texas", "texas-state": "Texas State University", classic: "UFCU Classic" } as Record<string, string>)[selectedCard]} /><ReviewDetail label="Selected priorities" value={preferences.map(preference => preferenceLabels[preference]).join(' · ')} /></div></section>
