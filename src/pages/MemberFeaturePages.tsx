...
export function CardsWalletPage() {
  const { profile, cardActivated, activateMember } = useMember()
...
function OfferCard({ merchant, category, distance, expiration }: { merchant: string; category: string; distance: string; expiration: string }) {
  return <article className="offer-card"><span className="merchant-logo">{merchant[0]}</span><div><strong>{merchant}</strong><small>{category} · {distance}</small><p>Available offer · Expires {expiration}</p><span className="offer-terms">Review terms before visiting</span></div><Icon name="chevronRight" size={18} /></article>
}
